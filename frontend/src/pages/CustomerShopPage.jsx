    ...
    <div className={styles.page}>
      ...
      {orderSuccess && (
        <div className={styles.successBanner}>
          ...
          <div>
            <strong>Order placed!</strong> Your order <span className={styles.orderNum}>{orderSuccess.orderNumber}</span> has been confirmed.
          </div>
        ...
        <div className={styles.productsPanel}>
          ...
          <div className={styles.catPills}>
            {CATEGORIES.map(c => (
              <button
                key={c}
                className={`${styles.catPill} ${filterCat === c ? styles.activePill : ''}`}
          ...
          {loading ? (
            <div className={styles.grid}>
              {[...Array(9)].map((_, i) => <div key={i} className={`skeleton ${styles.skCard}`} />)}
            </div>
          ) : (
            <div className={styles.grid}>
              {products.map(p => {
                ...
                return (
                  <div key={p._id} className={styles.productCard}>
                    ...
                    <div className={styles.productInfo}>
                      ...
                      <div className={styles.productPrice}>${p.price.toFixed(2)} <span>/ {p.unit}</span></div>
            ...
            <>
                ...
                {cartItems.map(item => (
                  <div key={item._id} className={styles.cartItem}>
                    ...
                    <div className={styles.cartItemInfo}>
                      ...
                      <div className={styles.cartItemPrice}>${item.price.toFixed(2)} × {item.qty}</div>
                    </div>
                    <div className={styles.cartItemTotal}>${(item.price * item.qty).toFixed(2)}</div>
                    <button className={styles.cartRemove} onClick={() => {
                      const updated = { ...cart };
              ...
              <div className={styles.cartFooter}>
                <div className={styles.cartTotalRow}>
                  ...
                  <span className={styles.cartTotalVal}>${cartTotal.toFixed(2)}</span>
                </div>
                <button
