    ...
    <div className={styles.page}>
      ...
      {loading ? (
        <div className={styles.skeletonGrid}>
          {[...Array(8)].map((_, i) => <div key={i} className={`skeleton ${styles.skCard}`} />)}
        </div>
      ) : (
        <div className={styles.grid}>
          {products.map(p => (
            <div key={p._id} className={`${styles.card} ${!p.isAvailable ? styles.unavailable : ''}`}>
              ...
              <div className={styles.cardBody}>
                ...
                <div className={styles.cardPrice}>${p.price.toFixed(2)} <span>/ {p.unit}</span></div>
          ...
          {products.length === 0 && (
            <div className={styles.empty}>No products found. <button onClick={openAdd}>Add one?</button></div>
          )}
        </div>
      )}
